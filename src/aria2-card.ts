...
export class Aria2Card extends LitElement {
    ...
    if (!('entry_id' in this.config)) {
      return html`<ha-card>
        <div class="card-content">
          You should edit the card to select the aria server to use.
        </div>
      </ha-card>`;
    }
        ...
        <div class="card-content">
          <div class="start-download-row">
            <ha-textfield
              ...
              class="addBox"
              iconTrailing="true"
              placeholder="${localize('card.url_to_download')}"
            ...
            ${repeat(
              ...
              (download: Download) => html`
                <div class="item">
                  ...
                  <div class="info">
                    <span
                      @click="${() => this.openDetail(download)}"
                      class="name"
                      >${download.name}</span
                    >
  ...
  async uploadFile(event: InputEvent & {target: HTMLInputElement}) {
    ...
    if (files.length > 0) {
      const fileContent = (await readFile(files[0])).split(',')[1];
      await this.hass.callService('aria2', 'start_download', {
        url: fileContent,
        server_entry_id: this.entryId,
      });
    }
  }

  openDetail(download: Download) {
    const hassShadowRoot =
      document.getElementsByTagName('home-assistant')[0].shadowRoot;

    const existingDownloadingDialog: DownloadDetailDialog | null | undefined =
      hassShadowRoot?.querySelector('download-detail-dialog');
  ...
  buildDownloadIcon(download: Download) {
    if (download.status === 'complete' || download.status === 'removed') {
      ...
      return html`
        <ha-icon-button
          @click="${() => this.actionOnDownload('resume', download)}"
          ><ha-icon style="display: flex" icon="hass:play"></ha-icon
        ></ha-icon-button>
        <ha-icon-button
          @click="${() => this.actionOnDownload('remove', download)}"
          ><ha-icon style="display: flex" icon="hass:stop"></ha-icon
        ></ha-icon-button>
      `;
    } else {
      return html`
        <ha-icon-button
          @click="${() => this.actionOnDownload('pause', download)}"
          ><ha-icon style="display: flex" icon="hass:pause"></ha-icon
        ></ha-icon-button>
        <ha-icon-button
          @click="${() => this.actionOnDownload('remove', download)}"
          ><ha-icon style="display: flex" icon="hass:stop"></ha-icon
  ...
  buildProgressBar(download: Download) {
    if (
      ...
      (!download.is_torrent || !download.seeder)
    ) {
      return html`
        <div
          class="progress"
  ...
  async startDownload() {
    if (
      ...
      this._downloadItem.value.length > 0
    ) {
      await this.hass.callService('aria2', 'start_download', {
        url: this._downloadItem.value,
        server_entry_id: this.entryId,
      });
      this._downloadItem.value = '';
    }
  ...
  async actionOnDownload(
    action: 'pause' | 'remove' | 'resume',
    download: Download
  ) {
    await this.hass.callService('aria2', action + '_download', {
      gid: download.gid,
      server_entry_id: this.entryId,
    });
  }
  ...
  async _refresh(aria_state: {server_entry_id?: string; list: Download[]}) {
    if (
      aria_state.server_entry_id &&
      this.entryId !== aria_state.server_entry_id
    ) {
      return;
    }
    ...
    if (existingDownloadingDialog?.currentDownload) {
      const refreshedDownload = this.downloads.filter(
        (download) =>
          download.gid == existingDownloadingDialog?.currentDownload?.gid
      );
